layui.define(['table', 'jquery', 'element','nyuwa'], function (exports) {
	"use strict";
	...
	function getData(url) {
		var defer = $.Deferred();
		nyuwa.request(url,{"type":"D,M"},function (res) {
			if (res.success){
	...
	function createMenu(option) {
		...
		$.each(option.data, function (i, item) {
			...
			if (item.openType == "_blank" && item.type == "M") {
				href = item.href;
	...
	function createMenuAndControl(option) {
		...
		$.each(option.data, function (i, item) {
			...
			$.each(item.children, function (i, note) {
				...
				var target = "";
				var className = "site-demo-active";
				if (note.openType == "_blank" && note.type == "M") {
					href = note.href;
					target = "target='_blank'";
					className = "";
				}
	...
	function downShow(option) {
		$("body #" + option.elem).on("click", "a[menu-type='D']", function () {
